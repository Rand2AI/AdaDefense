...
save_path = f"/home/hans/WorkSpace/Models/AdaDefense/GGL/GGL-{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"
if with_ad:
    save_path += f"-with_ad"
else:
    save_path += f"-no_ad"
...
for idx in idxes:
    for i in range(repeat):
        ...
        ng_rec = NGReconstructor(
            fl_model=model,
            generator=generator,
            loss_fn=loss_fn,
            with_kl=with_kl,
            G_key=G_key,
            with_lock_layer=with_lock_layer,
            shape_img=shape_img,
        ...
        plt.imsave(
            os.path.join(save_path, f"{idx}_true.png"),
            cv2.resize(original_img, shape_img, interpolation=cv2.INTER_CUBIC),
        )
        plt.imsave(
            os.path.join(save_path, f"{idx}_out_{i}.png"), cv2.resize(res_img, shape_img, interpolation=cv2.INTER_CUBIC)
        )
