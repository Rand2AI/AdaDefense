...
class Quantitation(object):
    ...
    def MSE(x, y):
        ...
        """
        return np.sqrt(np.mean((x.astype(np.float32) - y.astype(np.float32)) ** 2))
    ...
    def PSNR(x, y, factor=255.0):
        ...
        """
        mse = np.sqrt(np.mean((x.astype(np.float32) - y.astype(np.float32)) ** 2))
    ...
    def SSIM(x, y):
        ...
        @:keyword: higher, better
        """
        return structural_similarity(x, y, multichannel=True, channel_axis=2)
...
def GRNN():
    ...
    method = "GRNN"  # GRNN
    network = "lenet"  # lenet res20 res18
    datasets = ["mnist", "cifar10", "cifar100"]
    # datasets = ["cifar10", "cifar100", "imagenet"]
    for dataset in datasets:
        root_paht = f"/home/hans/WorkSpace/Models/AdaDefense/{method}/{network}"
        mse = []
        psnr = []
        lpips_vgg = []
        lpips_alex = []
        ...
        print(
            f"{mse_mean} & {mse_kl_mean} & {psnr_mean} & {psnr_kl_mean} & {lpips_vgg_mean} & {lpips_vgg_kl_mean} & {lpips_alex_mean} & {lpips_alex_kl_mean} & {ssim_mean} & {ssim_kl_mean}"
        )
...
def IG():
    ...
    datasets = ["cifar10", "cifar100", "imagenet"]
    for dataset in datasets:
        root_paht = f"/home/hans/WorkSpace/Models/AdaDefense/IG/"
        mse = []
        psnr = []
        lpips_vgg = []
        lpips_alex = []
        ...
        fake_img = None
        for d in os.listdir(root_paht):
            if d.endswith(f"_{dataset}_{network}_output_ad.png"):
                fake_img_ad = cv2.imread(f"{root_paht}/{d}")
            elif d.endswith(f"_{dataset}_{network}_true_ad.png"):
                true_img_ad = cv2.imread(f"{root_paht}/{d}")
            if true_img_ad is not None and fake_img_ad is not None:
                mse_kl.append(evaluator.MSE(true_img_ad, fake_img_ad))
                psnr_kl.append(evaluator.PSNR(true_img_ad, fake_img_ad))
                lpips_score = evaluator.LPIPS(true_img_ad, fake_img_ad)
                lpips_vgg_kl.append(lpips_score[0])
                lpips_alex_kl.append(lpips_score[1])
                ssim_kl.append(evaluator.SSIM(true_img_ad, fake_img_ad))
                true_img_ad = None
                fake_img_ad = None

            if d.endswith(f"_{dataset}_{network}_output.png"):
                fake_img = cv2.imread(f"{root_paht}/{d}")
            elif d.endswith(f"_{dataset}_{network}_true.png"):
                true_img = cv2.imread(f"{root_paht}/{d}")
            if true_img is not None and fake_img is not None:
                mse.append(evaluator.MSE(true_img, fake_img))
                psnr.append(evaluator.PSNR(true_img, fake_img))
                lpips_score = evaluator.LPIPS(true_img, fake_img)
                lpips_vgg.append(lpips_score[0])
...
def GGL():
    root_paht = f"/home/hans/WorkSpace/Data/Models/FedKL/GGL/"
    evaluator = Quantitation()
    mse = []
    psnr = []
    lpips_vgg = []
...
def white_black():
    ...
    print(
        f"32: {format(mse_32, '.2f')} & {format(psnr_32, '.2f')} & {format(lpips_vgg_32, '.2f')} & {format(lpips_alex_32, '.2f')} & {format(ssim_32, '.3f')}"
    )
    print(
        f"256: {format(mse_256, '.2f')} & {format(psnr_256, '.2f')} & {format(lpips_vgg_256, '.2f')} & {format(lpips_alex_256, '.2f')} & {format(ssim_256, '.3f')}"
    )
