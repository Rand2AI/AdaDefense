...
def construct_dataloaders(dataset, defs, size, data_path="/home/hans/WorkSpace/Data/", shuffle=False, normalize=True):
    ...
    if dataset == "cifar10":
        ...
        path = "/home/hans/WorkSpace/Data/ILSVRC/2012"
    ...
    trainloader = torch.utils.data.DataLoader(
        trainset,
        batch_size=min(defs.batch_size, len(trainset)),
        shuffle=shuffle,
        drop_last=True,
...
def _build_cifar10(data_path, size, augmentations=True, normalize=True):
    ...
    if augmentations:
        transform_train = transforms.Compose(
            [transforms.RandomCrop(size, padding=4), transforms.RandomHorizontalFlip(), transform]
        )
...
def _build_celaba(data_path, img_size, augmentations=True, normalize=True):
    ...
    validset = torchvision.datasets.CelebA(
        data_path,
        split="test",
        download=False,
        transform=transforms.Compose(
