...
class local_update(object):
    def __init__(self, config, client_idx, dataset, data_idxs, model, test_loader, client_keys=None):
        ...
        self.lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(
            self.optimizer, milestones=[config["TRAIN"]["ROUNDS"] // 3, 2 * (config["TRAIN"]["ROUNDS"] // 3)], gamma=0.1
        )
    ...
    def train(self, weight_global):
        ...
        for epoch in range(self.config["FED"]["CLIENT_EPOCH"]):
            ...
            if self.client_keys is not None:
                val_epoch_loss_avg, val_epoch_acc_avg = tester(
                    self.model,
                    self.test_loader,
                    self.criterion,
                ...
                train_epoch_loss_avg, train_epoch_acc_avg = trainer(
                    self.model, self.train_loader, self.optimizer, self.criterion, self.config["TRAIN"]["BATCH_SIZE"]
                )
        ...
        if self.client_keys is not None:
            self.key_layer = {
                k: v.clone().detach() for k, v in local_weights.items() if "scale_fc" in k or "shift_fc" in k
            }
            for k, v in self.key_layer.items():
