...
def _build_cifar10(data_path, augmentations=True, normalize=True):
    ...
    if augmentations:
        transform_train = transforms.Compose(
            [transforms.RandomCrop(32, padding=4), transforms.RandomHorizontalFlip(), transform]
        )
...
def _build_mnist(data_path, augmentations=True, normalize=True):
    ...
    if augmentations:
        transform_train = transforms.Compose(
            [transforms.RandomCrop(28, padding=4), transforms.RandomHorizontalFlip(), transform]
        )
        trainset.transform = transform_train
    else:
        trainset.transform = transform
