...
class SelfAttn(nn.Module):
    ...
    def __init__(self, in_channels, eps=1e-12):
        ...
        self.snconv1x1_theta = snconv2d(
            in_channels=in_channels, out_channels=in_channels // 8, kernel_size=1, bias=False, eps=eps
        )
        self.snconv1x1_phi = snconv2d(
            in_channels=in_channels, out_channels=in_channels // 8, kernel_size=1, bias=False, eps=eps
        )
        self.snconv1x1_g = snconv2d(
            in_channels=in_channels, out_channels=in_channels // 2, kernel_size=1, bias=False, eps=eps
        )
        self.snconv1x1_o_conv = snconv2d(
            in_channels=in_channels // 2, out_channels=in_channels, kernel_size=1, bias=False, eps=eps
        )
...
class BigGANBatchNorm(nn.Module):
    ...
    def forward(self, x, truncation, condition_vector=None):
        ...
        if self.conditional:
            ...
            out = F.batch_norm(
                x, running_mean, running_var, self.weight, self.bias, training=False, momentum=0.0, eps=self.eps
            )
...
class GenBlock(nn.Module):
    def __init__(
        self, in_size, out_size, condition_vector_dim, reduction_factor=4, up_sample=False, n_stats=51, eps=1e-12
    ):
...
if __name__ == "__main__":
    ...
    if not load_cache:
        model = load_tf_weights_in_biggan(
            model, config, "./models/model_128/", "./models/model_128/batchnorms_stats.bin"
        )
