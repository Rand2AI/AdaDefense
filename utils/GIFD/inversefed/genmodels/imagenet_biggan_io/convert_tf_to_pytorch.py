...
def extract_batch_norm_stats(tf_model_path, batch_norm_stats_path=None):
    try:
        ...
        raise ImportError(
            "Loading a TensorFlow models in PyTorch, requires TensorFlow and TF Hub to be installed. "
...
def build_tf_to_pytorch_map(model, config):
    ...
    tf_to_pt_map.update(
        {
            "linear/w/ema_0.9999": model.embeddings.weight,
    ...
    for i, (up, in_channels, out_channels) in enumerate(config.layers):
        ...
        for i in range(4):  #  Batchnorms
            ...
            for name in ("offset", "scale"):
                ...
                tf_to_pt_map.update(
                    {
                        sub_module_str + "w/ema_0.9999": sub_module_pnt.weight_orig,
        ...
        for i in range(4):  # Convolutions
            ...
            tf_to_pt_map.update(
                {
                    conv_str + "b/ema_0.9999": conv_pnt.bias,
    ...
    for pt_name, tf_name in zip(
        ["snconv1x1_g", "snconv1x1_o_conv", "snconv1x1_phi", "snconv1x1_theta"], ["g/", "o_conv/", "phi/", "theta/"]
    ):
    ...
    tf_to_pt_map.update(
        {layer_str + "offset/ema_0.9999": layer_pnt.bias, layer_str + "scale/ema_0.9999": layer_pnt.weight}
    )
    layer_str = "Generator/conv_to_rgb/"
    layer_pnt = model.generator.conv_to_rgb
    tf_to_pt_map.update(
        {
            layer_str + "b/ema_0.9999": layer_pnt.bias,
...
def load_tf_weights_in_biggan(model, config, tf_model_path, batch_norm_stats_path=None):
    ...
    remaining_params = list(
        n for n, t in chain(model.named_parameters(), model.named_buffers()) if t.data_ptr() not in pt_params_pnt
    )
...
def main():
    ...
    parser.add_argument("--model_type", type=str, default="", required=True, help="BigGAN model type (128, 256, 512)")
    parser.add_argument(
        "--tf_model_path", type=str, default="", required=True, help="Path of the downloaded TF Hub model"
    )
    parser.add_argument(
        "--pt_save_path",
        type=str,
    ...
    parser.add_argument(
        "--batch_norm_stats_path", type=str, default="", help="Path of previously extracted batch norm statistics"
    )
