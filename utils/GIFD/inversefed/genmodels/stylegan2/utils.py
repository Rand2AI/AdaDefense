...
class AttributeDict(dict):
    ...
    def __repr__(self):
        return "{}({})".format(
            self.__class__.__name__, ", ".join("{}={}".format(key, value) for key, value in self.items())
        )
...
class Timer:
    ...
    def __str__(self):
        ...
        if not value or value >= 100:
            ...
            return "{:.3g} us".format(value * 1e6)
        elif value >= 1e-9:
            return "{:.3g} ns".format(value * 1e9)
        else:
...
def slerp(a, b, beta):
    assert a.size() == b.size(), "Size mismatch between " + "slerp arguments, received {} and {}".format(
        a.size(), b.size()
    )
...
class ConfigArgumentParser(argparse.ArgumentParser):
    ...
    def parse_args(self, args=None):
        config_args = _parse_configs(getattr(super(ConfigArgumentParser, self).parse_args(args), self._CONFIG_ARG_KEY))
...
class ResizeTransform:
    def __init__(self, height, width, resize=True, mode="bicubic"):
        if resize:
            assert height and width, "Height and width have to be given " + "when resizing data."
        self.height = height
        self.width = width
        self.resize = resize
        self.mode = mode
            ...
            if tensor.size(1) != self.height or tensor.size(2) != self.width:
                if self.resize:
                    ...
                    tensor = F.interpolate(
                        tensor.unsqueeze(0), size=(self.height, self.width), mode=self.mode, **kwargs
                    ).squeeze(0)
                else:
                    raise ValueError(
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(ImageFolder, self).__init__(
            *args, loader=_PIL_grayscale_loader if grayscale else _PIL_RGB_loader, **kwargs
        )
        transforms = []
        if mirror:
        ...
        transforms.append(
            torchvision.transforms.Normalize(
                mean=[-(pixel_min / (pixel_max - pixel_min))], std=[1.0 / (pixel_max - pixel_min)]
            )
        )
...
class MovingAverageModule:
    def __init__(self, from_module, to_module=None, param_beta=0.995, buffer_beta=0, device=None):
...
class ProgressWriter:
    ...
    def __init__(self, length=None, progress_bar=True, clear=False):
        ...
        if is_notebook():
            self._writer = notebook_display(_StrRepr(""), display_id=time.asctime())
        else:
            if progress_bar:
...
def stack_images_PIL(imgs, shape=None, individual_img_size=None):
    ...
    canvas = Image.new("RGB", (shape[0] * width, shape[1] * height), (0, 0, 0, 0))
    imgs = imgs.copy()
