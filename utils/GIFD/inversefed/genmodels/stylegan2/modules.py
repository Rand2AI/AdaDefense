...
class DenseLayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, lr_mul=1, weight_scale=True, gain=1, *args, **kwargs):
        ...
        weight, weight_coef = _get_weight_and_coef(
            shape=[out_features, in_features], lr_mul=lr_mul, weight_scale=weight_scale, gain=gain
        )
        ...
        **kwargs
        ...
        if modulate:
            self.dense = BiasActivationWrapper(
                layer=DenseLayer(
                    in_features=latent_size, out_features=in_channels, lr_mul=lr_mul, weight_scale=weight_scale, gain=1
                ),
        ...
        **kwargs
        ...
        if fused:
            assert mode in ["FIR", "none"], (
                "Fused conv upsample can only use " + "'FIR' or 'none' for resampling " + "(`mode` argument)."
            )
        ...
        if self.fused:
            if self.modulate:
                weight = _setup_mod_weight_for_t_conv(
                    weight=weight, in_channels=self.in_channels, out_channels=self.out_channels
                )
            pad_out = False
...
class ConvDownLayer(ConvLayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if fused:
            assert mode in ["FIR", "none"], (
                "Fused conv downsample can only use " + "'FIR' or 'none' for resampling " + "(`mode` argument)."
            )
        ...
        **kwargs
        ...
        while len(self.conv_block) < num_layers:
            ...
            self.conv_block.append(
                _get_layer(ConvUpLayer if use_up else ConvLayer, layer_kwargs, wrap=True, noise=noise)
            )
    ...
    def __init__(
        ...
        **kwargs
        ...
        while len(self.conv_block) < num_layers:
            ...
            self.conv_block.append(
                _get_layer(ConvDownLayer if use_down else ConvLayer, layer_kwargs, wrap=True, noise=False)
            )
