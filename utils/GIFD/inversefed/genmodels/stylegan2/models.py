...
class _BaseModel(nn.Module):
    ...
    def load(cls, fpath, map_location="cpu"):
        ...
        assert isinstance(model, cls), "Trying to load a `{}` ".format(type(model)) + "model from {}.load()".format(
            cls.__name__
        )
        return model
...
class Generator(_BaseModel):
    ...
    def forward(
        ...
        if num_latents == 1:
            ...
            assert dlatents.size(1) <= len(self), "More latents ({}) than number ".format(
                dlatents.size(1)
            ...
            if not latent_to_layer_idx:
                ...
                dlatents = [
                    dlatents[:, i].unsqueeze(1).expand(-1, cutoffs[i + 1] - cutoffs[i], dlatents.size(2))
...
class _BaseParameterizedModel(_BaseModel):
    ...
    def __init__(self, **kwargs):
        ...
        self._update_default_kwargs(activation="lrelu:0.2", lr_mul=1, weight_scale=True, eps=1e-8)
        self._update_kwargs(**kwargs)
    ...
    def forward(self, latents, labels=None):
        ...
        if labels is not None:
            ...
            assert len(labels) == len(
                latents
            ), "Received different number of labels " + "({}) and latents ({}).".format(len(labels), len(latents))
            if not torch.is_tensor(labels):
                labels = torch.tensor(labels, dtype=torch.int64)
...
class GeneratorSynthesis(_BaseAdverserialModel):
    ...
    def __init__(self, **kwargs):
        ...
        self._update_default_kwargs(
            latent_size=512, demodulate=True, modulate_data_out=True, noise=True, resnet=False, skip=True
        )
        self._update_kwargs(**kwargs)
        ...
        for i in range(1, len(self.channels)):
            self.conv_blocks.append(
                modules.GeneratorConvBlock(
                    in_channels=self.channels[-i], out_channels=self.channels[-i - 1], **conv_block_kwargs
                )
            )
