...
def _grad(input, output, retain_graph):
    ...
    retain_graph = True
    grads = torch.autograd.grad(output.sum(), input, only_inputs=True, retain_graph=retain_graph, create_graph=True)
    return grads[0]
...
def G_logistic(G, D, latents, latent_labels=None, *args, **kwargs):
...
def G_logistic_ns(G, D, latents, latent_labels=None, *args, **kwargs):
    fake_scores = D(G(latents, labels=latent_labels), labels=latent_labels).float()
...
def D_logistic(G, D, latents, reals, latent_labels=None, real_labels=None, *args, **kwargs):
...
def D_r2(D, G, latents, latent_labels=None, gamma=10, *args, **kwargs):
    loss = None
    reg = None
    if gamma:
        with torch.no_grad():
...
def D_logistic_r1(G, D, latents, reals, latent_labels=None, real_labels=None, gamma=10, *args, **kwargs):
    assert (latent_labels is None) == (real_labels is None)
    with torch.no_grad():
        fakes = G(latents, labels=latent_labels)
    if gamma:
...
def D_logistic_r2(G, D, latents, reals, latent_labels=None, real_labels=None, gamma=10, *args, **kwargs):
    assert (latent_labels is None) == (real_labels is None)
    with torch.no_grad():
        fakes = G(latents, labels=latent_labels)
    if gamma:
...
def G_pathreg(G, latents, pl_avg, latent_labels=None, pl_decay=0.01, gamma=2, *args, **kwargs):
    loss = None
    reg = None
    if gamma:
        fakes, dlatents = G(latents, labels=latent_labels, return_dlatents=True, mapping_grad=False)
        reg = _pathreg(
            dlatents=dlatents, fakes=fakes, pl_avg=pl_avg, pl_decay=pl_decay, gamma=gamma, retain_graph=False
        ).float()
    return loss, reg
...
def G_logistic_ns_pathreg(G, D, latents, pl_avg, latent_labels=None, pl_decay=0.01, gamma=2, *args, **kwargs):
    ...
    if gamma:
        reg = _pathreg(
            dlatents=dlatents, fakes=fakes, pl_avg=pl_avg, pl_decay=pl_decay, gamma=gamma, retain_graph=True
        ).float()
    return loss, reg
...
def G_wgan(G, D, latents, latent_labels=None, *args, **kwargs):
    fake_scores = D(G(latents, labels=latent_labels), labels=latent_labels).float()
...
def D_wgan(G, D, latents, reals, latent_labels=None, real_labels=None, drift_gamma=0.001, *args, **kwargs):
    assert (latent_labels is None) == (real_labels is None)
    with torch.no_grad():
        fakes = G(latents, labels=latent_labels)
    real_scores = D(reals, labels=real_labels).float()
...
def D_gp(G, D, latents, reals, latent_labels=None, real_labels=None, gamma=0, constraint=1, *args, **kwargs):
    loss = None
    reg = None
    if gamma:
        assert (latent_labels is None) == (real_labels is None)
        ...
        reg = _grad_pen(input=interp, output=interp_scores, gamma=gamma, retain_graph=False).float()
    return loss, reg
...
def D_wgan_gp(
    ...
    **kwargs
    ...
    if gamma:
        ...
        interp = utils.lerp(reals, fakes, alpha).requires_grad_(True)
        interp_scores = D(interp, labels=latent_labels)
        reg = _grad_pen(input=interp, output=interp_scores, gamma=gamma, retain_graph=True).float()
    return loss, reg
