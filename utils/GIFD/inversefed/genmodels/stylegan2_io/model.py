...
class ConstantInput(nn.Module):
    def __init__(self, channel, size=4):
        ...
        self.input = nn.Parameter(torch.randn(1, channel, size, size))  # 512 x 4 x4
...
class Generator(nn.Module):
    def __init__(
        ...
        for layer_idx in range(self.num_layers):  # 17
            res = (layer_idx + 5) // 2
            shape = [1, 1, 2**res, 2**res]
            self.noises.register_buffer(f"noise_{layer_idx}", torch.randn(*shape))

        for i in range(3, self.log_size + 1):  # 16
    ...
    def forward(
        ...
        if not input_is_latent:
            styles = [self.style(s) for s in styles]  # A operation, standing for the affine transformation.
        ...
        if self.start_layer == 0:
            ...
            )  # 0th layer is special because here is only one conv and one noise vector.
