...
def gen_dataset(dataset, shape_img, data_path):
    if dataset == "mnist":
        ...
        tt_tst = transforms.Compose(
            [transforms.Resize(shape_img), transforms.ToTensor(), transforms.Normalize(mean, std)]
        )
...
def untar_ILSVRC2012():
    ...
    train_path = "/home/hans/WorkSpace/Data/ILSVRC/2012/Train/"
    ...
    for file in process_bar:
        ...
        for idx, name in enumerate(names):
            ...
            process_bar.set_postfix(file=file, progress=f"{idx + 1}/{len(names)}")
        tar.close()
