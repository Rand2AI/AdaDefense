...
def Normal(config):
    ...
    if "lenet" in model_name:
        model_name = "lenet"
    save_path = (
        config["TRAIN"]["SAVE_ROOT"]
        + f"{config['NAME']}/{config['METHODS']}/{config['METHODS']}-{model_name}-{config['DATA']['TRAIN_DATA']}-B{str(batchsize).zfill(3)}-{modelID}"
    )
    client_key = None
    if not config["DEBUG"]:
    ...
    for epoch in range(epochs):
        ...
        train_epoch_loss_avg, train_epoch_acc_avg = trainer(
            model, train_loader, optimizer, criterion, batchsize, client_key
        )
