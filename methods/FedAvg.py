...
def FedAvg(config):
    ...
    if config["DEVICE"]["DEVICE_TOUSE"] == "GPU":
        ...
        if len(config["DEVICE"]["DEVICE_GPUID"]) > 1:
            model_global = torch.nn.DataParallel(
                model_global, device_ids=list(range(len(config["DEVICE"]["DEVICE_GPUID"])))
            )
    ...
    if "lenet" in model_name:
        model_name = "lenet"
    save_path = f"/home/hans/WorkSpace/Models/{config['NAME']}/{config['METHODS']}/{config['METHODS']}-{model_name}-{config['DATA']['TRAIN_DATA']}-B{str(batchsize).zfill(3)}-{modelID}"
    if not config["DEBUG"]:
